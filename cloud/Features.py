...
if __name__ == '__main__':
    ...
    data = pd.read_csv('E:\VS codew\云计算\实验五\iris.txt', header=None)
    ...
    if stype == 'pca':
        pca = PCA(n_components=2, whiten=True, random_state=0)  #whiten：PCA白化也是一种常用的数据预处理，均值调整至0为中心，方差归一化
        ...
        fs.fit(x, y)
        idx = fs.get_support(indices=True) #设定indices=True，就可以确定被选择出来的特征在原特征矩阵中所在的位置的索引。
    ...
    model = Pipeline([
        ...
        ('lr', LogisticRegressionCV(Cs=np.logspace(-3, 4, 8), cv=5, fit_intercept=False,multi_class='ovr'))  #是否计算偏置
    ])
    model.fit(x, y)
