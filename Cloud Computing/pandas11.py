...
from fuzzywuzzy import process #用来返回模糊匹配的字符串和相似度。
...
def capital(str):
    return str.capitalize() #格式转换，首字母大写,其余小写
...
if __name__ == "__main__":
    ...
    data = pd.read_excel('E:\VS codew\云计算\实验六\sales.xlsx', sheet_name='sheet1', header=0)
    print('data.head() = \n', data.head())
    print('data.tail() = \n', data.tail())
    print('data.dtypes = \n', data.dtypes)
    print('data.columns = \n', data.columns)
    ...
    # 写入文件
    data.to_excel('E:\VS codew\云计算\实验六\sales_result.xls', sheet_name='Sheet1', index=False)
